...
> Full provenance for every figure below — dataset sizes, metric scopes, the
> decision trace and the replan scenario — is in **[EVIDENCE.md](EVIDENCE.md)**.
> This is a demonstration on a synthetic dataset with simulated operational
> events; it is not connected to any railway system.
...
> **Note on artifacts.** Four metric scopes are kept in separate files and must
> not be combined — see [EVIDENCE.md §1](EVIDENCE.md). The figures above are the
> full run (`benchmarks/full_run_metrics.json`). The root
> `optimization_metrics.json` and `optimized_block_plan.*` describe the smaller
> **demo scenario** that the guided walkthrough renders (53 of 62 tasks).
> `demo.py` writes to `demo_output/` and `demo_closed_loop.py` to
> `replan_output/`; neither overwrites the committed baseline.
...
- **Ritvik (Dynamic Operations & Replanning Engine):**
  ...
  - Time-interval collision detection (with a configurable safety buffer) between
    possessions and train movements, plus section capacity headroom analysis.
  - Chooses one of three operational actions, in order of preference:
    **REROUTE** the train (BFS over `route_topology.json`, delay computed from
    section length and line speed), **HOLD** the train if the wait is within
    `max_acceptable_hold_minutes` and it is not `priority_class` 1, or emit a
    **REPLAN** request to Arnav when neither is feasible.
  - Generates `ritvik_operational_decision.json` for Aditya (`PLAN_APPROVED` or
    `OPERATIONAL_UPDATE`) and `replan_request.json` back to Arnav.
  - Four worked scenarios (reroute / hold / replan / block-unavailable) are
    generated by `scripts/generate_ritvik_scenarios.py` and rendered on the
    Live Operations screen. Topology adjacency is a **22-of-200-section
    prototype**; conflicts elsewhere escalate to replanning.
  - Not implemented, not claimed: train sequencing, knock-on delay propagation.
